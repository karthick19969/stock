...
if (isset($_POST['submitButton']))
	...
	$query = "insert into Stocks_Sales(Stocks_Sales_Code,Stocks_Transactions_Id,Stocks_Sales_Date,Stocks_Sales_Amount,User,Payment,Balance,Bill_No,Customer) values('$stockSalesCode',$max_trans_id,'$today','$grandTotal','$user','$payment','$balance','$billNo',$customer)";
	$msg = mysqli_query($con, $query);
	$c = $_POST['checkyear'];
...
function addAnotherRow() {
    ...
    row.find('input[name="Reed_'+(oldId-1)+'"]').attr('id', 'Reed_' + oldId).attr('name', 'Reed_' + oldId);
    row.find('input[name="Pick_'+(oldId-1)+'"]').attr('id', 'Pick_' + oldId).attr('name', 'Pick_' + oldId);
    row.find('input[name="Warp_'+(oldId-1)+'"]').attr('id', 'Warp_' + oldId).attr('name', 'Warp_' + oldId);
    row.find('input[name="Wept_'+(oldId-1)+'"]').attr('id', 'Wept_' + oldId).attr('name', 'Wept_' + oldId);
    row.find('input[name="Width_'+(oldId-1)+'"]').attr('id', 'Width_' + oldId).attr('name','Width_' + oldId);
    row.find('input[name="SellingPrice_'+(oldId-1)+'"]').attr('id', 'SellingPrice_' + oldId).attr('name', 'SellingPrice_' + oldId);
...
function print1() {
	   ...
	   window.open("sales_print.php", "myNewWinsr", "width=620,height=800,toolbar=0,menubar=no,status=no,resizable=yes,location=no,directories=no,scrollbars=yes");
}
	        ...
	        $.ajax({
	            ...
	            success:function(data){
	                if(data.status == 'ok'){
	             	   ...
	             	   $row.find('input[name="Reed_'+(oldId-1)+'"]').val(data.result.Stocks_Reed);
	             	  $row.find('input[name="Pick_'+(oldId-1)+'"]').val(data.result.Stocks_Pick);
	             	 $row.find('input[name="Warp_'+(oldId-1)+'"]').val(data.result.Stocks_Warp);
	             	$row.find('input[name="Wept_'+(oldId-1)+'"]').val(data.result.Stocks_Wept);
	             	$row.find('input[name="Width_'+(oldId-1)+'"]').val(data.result.Stocks_Width);
	             	$row.find('input[name="SellingPrice_'+(oldId-1)+'"]').val(parseFloat(data.result.Selling_Price).toFixed(2));                 
	                }else{
												...
												<div class="form-group">
													...
													<div class="col-sm-8">
														...
														<input type="text" name="salesdate" id="test2"
															value="<?php
echo date('d-m-Y'); ?>" class="form-control"
															required="required">
												...
												<table  class="table table-bordered" id="tbl_posts">
													...
													<tbody id="tbl_posts_body">
														<tr id="rec-1">
															...
															<!-- td><span class="sn">1</span>.</td-->
															<td><select name="stockcode_0" class="changeStatus">
							<?php
...
?>						
	...
	echo $rowdropdown['Stocks_Name'];
															...
															<td><input type="text" required="required" id="Reed_0"
																name="Reed_0" class="reed"
																onkeypress="return numbersonly(event);" width="10%" /></td>
															<td><input type="text" required="required" id="Pick_0"
																name="Pick_0" class="pick"
																onkeypress="return numbersonly(event);" /></td>
															<td><input type="text" required="required" id="Warp_0"
																name="Warp_0" class="warp"
																onkeypress="return numbersonly(event);" /></td>
															<td><input type="text" required="required" id="Wept_0"
																name="Wept_0" class="wept"
																onkeypress="return numbersonly(event);" /></td>
															<td><input type="text" required="required" id="Width_0"
																name="Width_0" class="width"
																onkeypress="return numbersonly(event);" /></td>
															<td><input type="text" required="required"
																class="sellingPrice" id="SellingPrice_0"
																name="SellingPrice_0"
																onkeypress="return numbersonly(event);" /></td>
															<td><input type="text" required="required"
																class="quantity" id="Quantity_0" name="Quantity_0"
																onkeyup="getRows();grand_total();"
																onkeypress="return numbersonly(event)" /></td>
															<td><input name="Total_0" type="text" id="Total_0"
																class="total" maxlength="200" class="total" /></td>
...
echo date('d-m-Y'); ?>">
											...
											<div class="col-sm-8 col-sm-offset-2">
												<input class="btn btn-primary" type="submit"
													name="submitButton" value="Save" /> <input
